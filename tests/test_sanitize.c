...
static void test(const char *desc, bool ansi,
                 const char *input, size_t max_cols,
...
int main(void)
{
    ...
    test("printable text passes through",
         false, "hello", 80,
         "hello", 5);

    test("truncate at max_cols",
         false, "hello world", 5,
         "hello", 5);
    ...
    test("control char replaced with dot",
         false, "a\x01z", 80,
         "a.z", 3);

    test("DEL replaced with dot",
         false, "a\x7fz", 80,
         "a.z", 3);
    ...
    test("tab mid-line expands to next stop",
         false, "ab\t", 80,
         "ab      ", 8);

    test("ESC replaced with dot (default mode)",
         false, "\033[31m", 80,
         ".[31m", 5);
    ...
    test("ANSI: printable text passes through",
         true, "hello", 80,
         "hello\033[0m", 5 + 4);

    test("ANSI: CSI sequence passed through",
         true, "\033[31mred\033[0m", 80,
         "\033[31mred\033[0m\033[0m", 3 + 5 + 4 + 4);

    test("ANSI: CSI doesn't count as visible columns",
         true, "\033[31mred\033[0m", 3,
         "\033[31mred\033[0m", 5 + 3 + 4);

    test("ANSI: truncation counts only visible cols",
         true, "\033[31mhello world\033[0m", 5,
         "\033[31mhello\033[0m", 5 + 5 + 4);

    test("ANSI: two-byte escape passed through",
         true, "\033Mtext", 80,
         "\033Mtext\033[0m", 2 + 4 + 4);

    test("ANSI: control chars still replaced with dot",
         true, "a\x01z", 80,
         "a.z\033[0m", 3 + 4);

    test("ANSI: newline stripped",
         true, "abc\n", 80,
         "abc\033[0m", 3 + 4);

    test("ANSI: tab still expands",
         true, "\t", 80,
         "        \033[0m", 8 + 4);
